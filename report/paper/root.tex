...
We begin with an unoptimized topometric map, containing $N$, a set of $|N|$
interconnected nodes named $n_1, n_2, ... n_{|N|}$ and $t_1, t_2, ... t_{|N|-1}
\in T$ the transformations to go from the frame of one node to another.
the geometric transformations to go from one node to its neighbour. The nodes in $N$ are in fact
...
\begin{align*}
 &\argmin_{S \in \mathcal{P}(N)} |S|  \\
( \forall n_{j} &\in S \: | \: \texttt{LOCALISES}(n_{j-1}, n_{j})) \\
\texttt{LOCALISES}&(n_{j-1},n_{j}) \Leftrightarrow  \\
  &( \forall p \in P \: | \: \|\texttt{ICP}(n_{j-1}, n_{j}, p) - t_{j-1}\| < \epsilon) \\
  P &\Leftrightarrow \\ 
  &R(c \cos (v)) T(a \cos( u ) \sin( v ))t_{j-1}
\end{align*}
...
Lets consider a reading $r$ and an anchor point $a$. We know that $r$ is derived
from inducing an error $[x, y]^T$ to $p_r$, which is associated to a node $n_r$.
We also know that the anchor point $a$ is associated to a node $n_a$ and a point
cloud $p_a$. We can easily get the transform $t$ between $n_r$ and $n_a$ from
$T$.


We want to know if $a$ can be used to localize $r$. To do so, we run the ICP
algorithm using $r$ as the input and $p_a$ as the reference, using $t$ as the
initialization of the ICP. ICP returns a transform $[x_{icp}, y_{icp}]^T$. We
would expect that ICP corrects the induced error by returning a transformation
$[-x, -y]^T$. This is verified by checking if the norm of the vector $[x_{icp},
y_{icp}]^T - [-x, -y]^T$ is under a certain $\epsilon$.
